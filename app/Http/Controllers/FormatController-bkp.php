...
class FormatController extends Controller
{
    ...
    public function view_fieldaudit()
    {
        ...
        $quatdetails = FormatModel::fetch_quarterdetails($sessiondeptcode);
    ...
    public function get_listinstitututes(Request $request)
    {
        ...
        if ($formname == 'downloadreport') {
            ...
            $results = FormatModel::fetch_listinstitutes($session_userid, 'finalized', $instid, $whom, $quarterode);
        } else {
            $results = FormatModel::fetch_listinstitutes($session_userid, '', '', 'AH', $quarterode);
        }
    ...
    //     $data = (array) FormatModel::GetSchedultedEventDetails($auditscheduleid, $spilloverflag, $instid, $financialyearcode); // Convert to array for merging
    ...
    private function loadAllValues($scheduleId, $lang, $spilloverflag, $instid, $financialyearcode)
    {
        ...
        //     if (empty($head_en) && empty($members_en)) continue;

        //     // Use English or Tamil based on $lang for HTML block
        //     $head    = ($lang === 'en') ? $head_en : $head_ta;
        //     $members = ($lang === 'en') ? $members_en : $members_ta;

        //     $TeamDetails .= $formatTeamDetails($head, $members, $teamHead_Label, $teamMem_Label, $teamIndex + 1);

        //     // Dynamic keys for each team
...
\t\t    <td style=\"width:12%;\">" . ($accountTypeMap[$account_type[$key] ?? ''] ?? '') . '</td>
 ...
 <b>' . $label_layout['financeyr'] . '- ' . $gst_audityear . '</b>
     ...
     public function DownloadAuditReport($scheduleId, $lang, $spilloverflag, $instid, $financialyearcode, $action = 'download')
    {
        ...
        if ($attachmentsOnly) {
            $mpdfContent = $this->generateFirstPageHtml($deptsize, $DeptName, $base64Image, $DistName, $InstituteName, $FinancialYear, $AuditReport_Year, $AuditReport_Text);
        }
	...
	$GetauditSlips = FormatModel::Fetchparadetails($scheduleId, $spilloverflag, $instid, $financialyearcode);
        ...
        if ($reportOnly) {
            ...
            try {
                if (!$this->mergePdfFilesWithGhostscript([$tempReportPdfPath], $reportCachePath, 'compressed')) {
                    throw new \RuntimeException('Ghostscript failed to optimize the audit report.');
                }
        ...
        try {
            if (!empty($slipAnnextureFiles)) {
                ...
                $this->collectAnnexurePdfParts($pdfParts, $tempPdfParts, $slipAnnexurePaths, ['slip_related' => 'Annexure'], 'slipbased', $lang, !$useFastAttachmentMode);
            }
            ...
            $this->collectAnnexurePdfParts($pdfParts, $tempPdfParts, $converted, $annexureLabels, 'fileupbased', $lang, !$useFastAttachmentMode);

            $attachmentMergeProfile = $useFastAttachmentMode ? 'fast' : 'attachments_final';
            if (!$this->mergePdfFilesWithGhostscript($pdfParts, $finalFilePath, $attachmentMergeProfile)) {
                throw new \RuntimeException('Ghostscript failed to merge the audit report attachments.');
            }
...
private function collectAnnexurePdfParts(array &$pdfParts, array &$tempPdfParts, array $annexureFiles, $labels, $typeofann, $lang, bool $includeTitlePages = true)
    ...
    private function mergePdfFilesWithGhostscript(array $inputPaths, string $outputPath, string $profile = 'compressed'): bool
    {
        $ghostscript = $this->getGhostscriptBinary();
        if ($ghostscript === null) {
            throw new \RuntimeException('Ghostscript executable not found. Configure GHOSTSCRIPT_BIN or install gs/gswin64c.');
        }

        $validPaths = array_values(array_filter($inputPaths, static function ($path) {
            return is_string($path) && File::exists($path);
        }));
        ...
        $chunkSize = $profile === 'fast' ? 35 : 20;
        $workingInputs = $validPaths;
        $tempChunks = [];

        try {
            while (count($workingInputs) > $chunkSize) {
                $nextLevelInputs = [];

                foreach (array_chunk($workingInputs, $chunkSize) as $chunk) {
                    $chunkOutputPath = $this->createTempPdfPath('gs_chunk_');
                    $tempChunks[] = $chunkOutputPath;

                    if (!$this->runGhostscriptMerge($ghostscript, $chunk, $chunkOutputPath, 'chunk_fast')) {
                        return false;
                    }

                    $nextLevelInputs[] = $chunkOutputPath;
                }

                $workingInputs = $nextLevelInputs;
            }

            return $this->runGhostscriptMerge($ghostscript, $workingInputs, $outputPath, $profile);
        } finally {
            foreach ($tempChunks as $tempChunk) {
                if (is_string($tempChunk) && File::exists($tempChunk)) {
                    @unlink($tempChunk);
                }
            }
        }
    ...
    private function runGhostscriptMerge(string $ghostscript, array $inputPaths, string $outputPath, string $profile): bool
    {
        $baseCommand = escapeshellarg($ghostscript)
        ...
        if ($profile === 'chunk_fast' || $profile === 'fast') {
            $baseCommand .= ' -dPDFSETTINGS=/default';
        } elseif ($profile === 'attachments_final') {
            $baseCommand .= ' -dPDFSETTINGS=/screen'
            ...
            $baseCommand .= ' -dPDFSETTINGS=/ebook'
        ...
        return $resultCode === 0 && File::exists($outputPath) && filesize($outputPath) > 0;
    }
    ...
    private function getGhostscriptBinary(): ?string
{
    ...
    foreach ($candidates as $candidate) {
        ...
        $command = escapeshellarg($candidate) . " -version 2>$nullDevice";
    ...
    // public function instfetchforreportgeneration(Request $request)
    // {

    //     $teempins = FormatModel::instfetchforreportgeneration('audit.templateauditplan');

    //     return response()->json([
    //         'success' => ! $teempins->isEmpty(),
    //         'message' => $teempins->isEmpty() ? 'User not found' : '',
    //         'data' => $teempins->isEmpty() ? null : $teempins,
    //     ], $teempins->isEmpty() ? 404 : 200);
    // }

    private function renderFirstPage($data)
    {
        return $this->renderTemplate('intropage.html', $data);
    }
    ...
    public function downloadTemplateReport(Request $request)
    {
        try {
            ...
            // === Prepare content ===
            $dynamicData = $this->prepareDynamicData($firstInst, $language);
            $entrySheetHtml = $this->EntrySheetdata($firstInst, $language, $formcode, $tempplanid, $catcode);
            $remarksSheetHtml = $this->Remarksdata($firstInst, $language, $formcode);

            $auditcertificate = $this->auditcertificatefortemplate($firstInst, $language, $formcode);
            // === Initialize PDF with Tamil font ===
            $mpdf = new \Mpdf\Mpdf([
                'mode' => 'utf-8',
                'format' => 'A4',
                'orientation' => 'P',
                ...
                'default_font' => $language === 'ta' ? 'latha' : 'times',
            ]);
            ...
            // === First Page ===
            $firstPageHtml = $this->renderFirstPage($dynamicData);
            ...
            $mpdf->WriteHTML('
                <div style=" position: absolute;
                            ...
                            left: 0%;
                             ...
                             text-align: center;
                             width: 100%;text-align:center; font-family:' . ($language === 'ta' ? 'latha' : 'times') . ';">
                    <h2 style="font-size:35px; margin:0;">2. ' . $title . '</h2>
                </div>
            ');
            $mpdf->Rect(5, 5, $mpdf->w - 10, $mpdf->h - 10);
            ...
            if ($formcode === 'HRIA01') {
                $mpdf->AddPage();
                $mpdf->WriteHTML('<div style=" position: absolute;
                             position: absolute;
                         top: 35%;
                         text-align: center;
                         transform: translate(50%, 50%);
                         width: 85%;font-family:' . ($language === 'ta' ? 'latha' : 'times') . ';">' . $entrySheetHtml . '</div>');
                $mpdf->Rect(5, 5, $mpdf->w - 10, $mpdf->h - 10);
            } else {
                $mpdf->WriteHTML('<div style="page-break-after: always;"></div>');
                $mpdf->WriteHTML('<div style="font-family:' . ($language === 'ta' ? 'latha' : 'times') . ';">' . $entrySheetHtml . '</div>');
                $mpdf->Rect(5, 5, $mpdf->w - 10, $mpdf->h - 10);
            }
            ...
            $mpdf->WriteHTML('<h2 style="text-align:center; margin-top:50px; font-size:35px; font-family:' . ($language === 'ta' ? 'latha' : 'times') . ';">3. ' . $title . '</h2>');
            $mpdf->WriteHTML('<div style="font-family:latha; unicode-bidi: plaintext;">' . $remarksSheetHtml . '</div>');
            $mpdf->Rect(5, 5, $mpdf->w - 10, $mpdf->h - 10);

            // ===== 4. AUDIT CERTIFICATE =====
            $title = $language === 'ta' ? $sections[4]['ta'] : $sections[4]['en'];

            $mpdf->WriteHTML('<h2 style="
            text-align:center; margin-top:50px; font-size:35px; font-family:' . ($language === 'ta' ? 'latha' : 'times') . ';">4. ' . $title . '</h2>');
            $mpdf->WriteHTML('<div style="font-family:' . ($language === 'ta' ? 'latha' : 'times') . ';">' . $auditcertificate . '</div>');
            $mpdf->Rect(5, 5, $mpdf->w - 10, $mpdf->h - 10);

            // ===== 5. ATTACHMENTS =====
            $entryData = FormatModel::remarkssheet($firstInst->instid, 'audit.templateauditplan', $formcode)->first();
            $title = $language === 'ta' ? $sections[5]['ta'] : $sections[5]['en'];

            if ($entryData && !empty($entryData->filepath)) {
                $filePath = storage_path('app/public/' . str_replace(['\\', '/'], DIRECTORY_SEPARATOR, $entryData->filepath));

                $mpdf->AddPage();
                $mpdf->WriteHTML('<h2 style=" position: absolute;
                     top: 50%;
                     left: 0%;
                     transform: translate(50%, 50%);
                     text-align: center;
                     width: 100%;text-align:center; font-size:35px; font-family:' . ($language === 'ta' ? 'latha' : 'times') . ';">5. ' . $title . '</h2>');
                $mpdf->Rect(5, 5, $mpdf->w - 10, $mpdf->h - 10);

                //  $mpdf->AddPage();
                if (file_exists($filePath)) {
                    $fixedPdfPath = $this->convertPdfTo14fortmp($filePath);

                    if (!$fixedPdfPath || !file_exists($fixedPdfPath)) {
                        throw new \Exception('Attachment PDF is corrupted or unsupported.');
                    }

                    $pageCount = $mpdf->SetSourceFile($fixedPdfPath);
                    ...
                    for ($i = 1; $i <= $pageCount; $i++) {
                        ...
                        $mpdf->UseTemplate($tplId, 0, 0, $mpdf->w, $mpdf->h, true);
                    }

                    @unlink($fixedPdfPath);
                } else {
                    $mpdf->WriteHTML('<p style="color:#888;">Attachment file not found.</p>');
                    $mpdf->AddPage();
                }
            } else {
                $mpdf->WriteHTML('<p style="color:#888;"></p>');
            }
            ...
            $mpdf->WriteHTML('<h2 style=" position: absolute;
                     ...
                     transform: translate(50%, 50%);
                     text-align: center;
                     width: 100%;text-align:center; font-size:30px; font-family:' . ($language === 'ta' ? 'latha' : 'times') . ';">**End of the Report**</h2>');
            $mpdf->Rect(5, 5, $mpdf->w - 10, $mpdf->h - 10);
            ...
            return response()->streamDownload(function () use ($mpdf) {
                echo $mpdf->Output('', 'S');  // output PDF content
            }, $fileName);
        } catch (\Exception $e) {
            return response()->json(['error' => $e->getMessage()], 500);
        }
    ...
    public function generateReport(Request $request)
    {
        ...
        $attachments = $this->Templateattachments($firstInst, $language, $formcode);
        ...
        foreach ([2, 3, 4, 5] as $secNum) {
            ...
            $reportHtml .= '<h2 style="text-align:center; margin-top:100px; font-size:35px;">'
                . $secNum . '. ' . htmlspecialchars($title)
                . '</h2>';

            switch ($secNum) {
                case 2:
                    $reportHtml .= $entrySheetHtml;
                    break;
                case 3:
                    $reportHtml .= $remarksSheetHtml;
                    break;
                case 4:
                    $reportHtml .= $auditcertificate;

                    break;
                case 5:
                    $reportHtml .= $attachments;
                    break;
            }
        ...
        // $reportHtml .= $this->renderAttachments($dynamicData);
        // $reportHtml .= $this->renderAuditCertificate($dynamicData);

        return response()->json(['html' => $reportHtml, 'encrypted' => $encryptedData]);
    }

    private function auditcertificatefortemplate($inst, $language, $formcode)
    {
        $entryData = FormatModel::remarkssheet($inst->instid, 'audit.templateauditplan', $formcode)->first();
    ...
    private function Templateattachments($inst, $language, $formcode)
    {
        $entryData = FormatModel::remarkssheet($inst->instid, 'audit.templateauditplan', $formcode)->first();

        // If there is no entry data or filepath is empty, return empty string
        if (!$entryData || empty($entryData->filepath)) {
            return '';
        }
        ...
        // --- Validation: Check file type (must be PDF) ---
        ...
        // --- Validation: Check file size (under 2 MB) ---
        ...
        $html .= '<div class="attachment-block" style="margin-bottom:20px;">';
        $html .= '<div id="pdf-viewer" style="border:1px solid #ccc;"></div>';
        ...
        $html .= '</div>';  // close attachment-block
        $html .= '</div>';  // close attachments-section

        return $html;
    }

    private function Remarksdata($inst, $language, $formcode)
    {
        $entryData = FormatModel::remarkssheet($inst->instid, $table = 'audit.templateauditplan', $formcode);
        $remarksList = collect($entryData)->pluck('remark_text', 'type')->filter();

        $html = '<div class="remarks-section">';

        foreach ($entryData as $remark) {
            $type = $remark->type ?? '';
            $remark_text = $remark->remark_text ?? '';

            // Build label only if type exists
    ...
    private function EntrySheetdata($inst, $language, $formcode, $tempplanid, $catcode)
    {
        ...
        // -------- HRIA01 logic --------
        if ($formcode === 'HRIA01') {
            $auditTemplates = FormatModel::getTemplates($deptcode, $formcode, $inst->instid, $tempplanid);
            $amounts = FormatModel::getdetails($inst->instid, $formcode);
            $templatesData = [];
            foreach ($auditTemplates as $temp) {
                $templatesData[] = [
                    'structure' => FormatModel::getTemplateStructure($temp->tmpaudittypeid, $inst->instid),
                ];
            }

            $var = self::structureHriaTemplate($templatesData, $amounts, $language);

            return $var;
        }
        // -------- HRIA02 logic --------
        elseif ($formcode === 'HRIA02') {
            $auditTemplates = FormatModel::getTemplates($deptcode, $formcode, $inst->instid, $tempplanid);
            $amounts = FormatModel::getdetails($inst->instid, $formcode);
            $templatesData = [];
            foreach ($auditTemplates as $temp) {
            ...
            $var = self::structureHriaFullReport($templatesData, $amounts, $language);

            return $var;
        } elseif ($formcode === 'MILK01') {
            $auditTemplates = FormatModel::getTemplates($deptcode, $formcode, $inst->instid, $tempplanid);
            $amounts = FormatModel::getdetails($inst->instid, $formcode);
            ...
            $var = self::milkstructuretemplate($templatesData, $amounts);

            return $var;
        } elseif ($formcode === 'DCA01' || $formcode === 'DCA02') {
            $auditTemplates = FormatModel::getTemplates($deptcode, $formcode, $inst->instid, $tempplanid, $catcode);
            $amounts = FormatModel::getdetails($inst->instid, $formcode);
            ...
            $var = self::structureDcaReport($templatesData, $amounts, $language);

            return $var;
        } elseif ($formcode === 'LFA01') {
            $auditTemplates = FormatModel::getTemplates($deptcode, $formcode, $inst->instid, $tempplanid);
            $amounts = FormatModel::getdetails($inst->instid, $formcode);
            //  dd($amounts);
            $templatesData = [];
            ...
            $var = self::structureLfaReport($templatesData, $amounts, $language);
    ...
    private function prepareDynamicData($inst, $language)
    {
        if ($language === 'ta') {
