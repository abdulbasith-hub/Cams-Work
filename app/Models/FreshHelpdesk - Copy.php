...
VALUES
    ...
    public static function ticketStatusLabel(?string $status): string
    {
        ...
        if (in_array($normalized, ['closed', 'resolved'], true)) {
            return 'Resolved';
        }
    ...
    public static function taskCurrentlyWith(object $task): string
    {
        ...
        if ($status === self::TASK_RESOLVED) {
            return !empty($task->verified_by) ? 'NIC Admin' : 'Senior Developer';
        }
    ...
    public static function isDashboardTaskUrgent(object $task): bool
    {
        ...
        try {
            return Carbon::parse($task->expected_date_to_complete)->lt(now('Asia/Kolkata'));
    ...
    {
        return str_contains(strtolower((string) ($task->task_status_by_tester ?? '')), 'returned');
    }
    ...
    private static function stateForwardedRoleValues(): array
    {
        return [
            'superadmin',
            'stateadmin',
    ...
    public static function forwardTicket(object $ticket, string $action, array $data): object
    {
        ...
        $workflowUpdates = [
            ...
            $comment = self::ticketActionComment('Returned to State Admin.', $remarks);
	            ...
	            $updates += $workflowUpdates + [
		                'status' => self::TICKET_RESOLVED,
		                'forwarded_to_role' => 'user',
		                'resolved_at' => $now,
		                'assigned_to_userid' => null,
		                'assigned_to_name' => null,
	            ];
	            $comment = self::ticketActionComment('Resolved and returned to User.', $remarks);
    ...
    public static function forwardTask(object $task, string $action, array $data): object
    {
        ...
        if ($action === 'developer_resolve') {
            ...
            $developerDefaultComment = $taskStatus === self::TASK_RESOLVED
                ? 'Task forwarded to Senior Developer for review.'
                : 'Developer updated the task status.';
    ...
    public static function ticketsForCurrentRole(): Collection
    {
        $role = self::role();
        $query = DB::table('audit.helpdesk_tickets')->orderByDesc('created_at')->orderByDesc('id');

        if ($role === self::ROLE_USER) {
            $query->where('cams_userid', self::userId());
        } elseif ($role === self::ROLE_STATE_ADMIN) {
            $query->whereIn('forwarded_to_role', array_merge(self::stateForwardedRoleValues(), ['user']));
        } elseif ($role === self::ROLE_NIC_ADMIN) {
            $query->where('forwarded_to_role', 'nicadmin');
        } elseif ($role === self::ROLE_SENIOR_DEVELOPER) {
            $query->where('forwarded_to_role', self::ROLE_SENIOR_DEVELOPER)
                ->where('assigned_to_userid', self::userId());
        } elseif ($role === self::ROLE_DEVELOPER) {
            $query->where('forwarded_to_role', self::ROLE_DEVELOPER)
                ->where('assigned_to_userid', self::userId());
        }
    ...
    public static function ticketDetailsForCurrentRole(array $filters = [], bool $dashboardMode = false): Collection
    {
        ...
        if ($role === self::ROLE_USER) {
            ...
            $query->where(function ($dashboardQuery) {
                $dashboardQuery->whereIn('t.forwarded_to_role', [
                    'nicadmin',
                    self::ROLE_NIC_ADMIN,
                    self::ROLE_SENIOR_DEVELOPER,
                    self::ROLE_DEVELOPER,
        ...
        if (self::canViewDeveloperStatus($role) && !empty($filters['developer_status']) && self::ensureDeveloperStatusColumn()) {
            $query->where('t.developer_status', (string) $filters['developer_status']);
        }
    ...
    private static function tasksForCurrentRoleQuery(bool $dashboardMode = false, array $filters = [])
    {
        ...
        if ($dashboardMode && in_array($role, [self::ROLE_NIC_ADMIN, self::ROLE_SENIOR_DEVELOPER], true)) {
            ...
            $query->where(function ($taskQuery) use ($developerUserId, $hasTaskHistoryTable) {
                $taskQuery->where('developer_userid', $developerUserId)
            ...
            $query->whereIn('task_status_by_tester', [self::TASK_NOT_STARTED, self::TASK_IN_PROGRESS])
    ...
    public static function tasksForCurrentRole(bool $dashboardMode = false, array $filters = []): Collection
    {
        return self::tasksForCurrentRoleQuery($dashboardMode, $filters)->limit(100)->get();
    }
    ...
    public static function ticket(int $id): object
    {
        ...
        return DB::table('audit.helpdesk_tickets as t')
            ->leftJoinSub($financialYears, 'fy', 'fy.financialyearcode', '=', 't.financialyearcode')
            ->leftJoinSub($planMappings, 'apm', 'apm.planmappingid', '=', 't.planmappingid')
            ->leftJoinSub($auditQuarters, 'aq', 'aq.auditquartercode', '=', 'apm.auditquartercode')
            ->leftJoinSub($latestAssignments, 'latest_assignment', 'latest_assignment.ticket_id', '=', 't.id')
    ...
    {
        return DB::table(self::TASK_TABLE)->where('id', $id)->firstOrFail();
    }
    ...
    public static function activeDevelopers(): Collection
    {
	    ...
	    {
	        $chargeId = (string) (session('charge')->chargeid ?? '1');
    ...
    public static function recipientEmailsForTicket(object $ticket): array
    {
        ...
        return match ($forwardedRole) {
            ...
            self::ROLE_SENIOR_DEVELOPER, self::ROLE_DEVELOPER => [self::developerById($ticket->assigned_to_userid)->email ?? null],
    ...
    public static function recipientEmailsForTask(object $task): array
    {
        ...
        return [self::developerById($task->developer_userid ?? null)->email ?? null];
    }
    ...
    private static function latestTaskSenior(int $taskId): ?object
    {
        if (!self::ensureTaskHistoryTable()) {
            return null;
        }
        ...
        return DB::table(self::TASK_HISTORY_TABLE)
            ->where('task_id', $taskId)
            ->where('action_key', 'senior_to_developer')
    ...
    private static function isSeniorDeveloper(string $userId): bool
    {
        return DB::table('audit.dev_userdetails')
