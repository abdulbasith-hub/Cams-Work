...
<div class="row">
    <div class="col-12">
        <div class="card card_border">
            <div class="card-header card_header_color lang" key="">HRIA Zone </div>
            <div class="card-body">
                <form id="zoneform" name="zoneform">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            <label class="form-label required lang" key="zoneename" for="zoneename">Zone English
                                name</label>
                            <input type="text" data-placeholder-key="zoneename" class="form-control removesplchar_text"
                                id="zoneename" name="zoneename" required />
                        </div>
                        <div class="col-md-4 mb-3">
                            <label class="form-label required lang" key="zonetname" for="zonetname">Zone Tamil
                                name</label>
                            <input type="text" class="form-control removesplchar_text" id="zonetname" name="zonetname"
                                data-placeholder-key="zonetname" required />
                        </div>
    ...
    <div class="card card_border">
        <div class="card-header card_header_color lang" key="">Zone Details</div>
                    ...
                    <table id="zonetable"
                        ...
                        <thead>
                            <tr>
                                ...
                                <th class="lang align-middle text-center" key="zoneename">Zone Name In English
                                </th>
                                <th class="lang align-middle text-center" key="zonetname"> Zone Name In Tamil </th>
...
<script>
    ...
    $('#translate').change(function() {
        ...
        updateValidationMessages(getLanguage('Y'), 'zoneform');
    });

    function initializeDataTable(language) {
        $.ajax({
            url: "{{ route('zone_fetchData') }}",
            type: "POST",
            headers: {
    ...
    function renderTable(language) {
        ...
        if ($.fn.DataTable.isDataTable('#zonetable')) {
            $('#zonetable').DataTable().clear().destroy();
        }
        ...
        var table = $('#zonetable').DataTable({
            ...
            columns: [{
                ...
                {
                    data: "zoneename",
                    title: columnLabels?.["zoneename"]?.[language],
                    className: "d-none d-md-table-cell lang extra-column ",
                    render: function(data, type, row) {
                        return row.zoneename || '-';
                    }
                },
                {
                    data: "zonetname",
                    title: columnLabels?.["zonetname"]?.[language],
                    className: "d-none d-md-table-cell lang extra-column ",
                    render: function(data, type, row) {
                        return row.zonetname || '-';
                    }
                },
        ...
        const mobileColumns = ["zoneename", "zonetname", "statusflag"];
        setupMobileRowToggle(mobileColumns);
        updatedatatable(language, "zonetable");
    }
    ...
    jsonLoadedPromise.then(() => {
        ...
        var validator = $("#zoneform").validate({
            rules: {
                distcode: {
                    required: true,
                },
        ...
        $("#buttonaction").on("click", function(event) {
            ...
            if ($("#zoneform").valid()) {
                ...
                var formData = $('#zoneform').serializeArray();
                $.ajax({
                    url: "{{ route('zone_insertupdate') }}",
                    type: 'POST',
                    data: formData,
                ...
                success: function(response) {
                    if (response.success) {
                        if (response.data && response.data.length > 0) {
                            changeButtonAction('zoneform', 'action', 'buttonaction',
                                'reset_button', 'display_error', @json($updatebtn),
                                @json($clearbtn), @json($update))
                            populatezoneform(response.data[0]); // Populate form with data
                        } else {
...
$(selector)
    ...
    function populatezoneform(zone) {
        $('#display_error').hide();
        $('#zoneename').val(zone.zoneename);
        $('#zonetname').val(zone.zonetname);
        populateStatusFlag(zone.statusflag);
        $('#distcode').val(zone.distcode).change();
        updateSelectColorByValue(document.querySelectorAll(".form-select"));
    }
    ...
    function reset_form() {
        if (sessiondeptcode && sessiondeptcode.trim() !== '') {
            ...
            $('#zoneename,#zonetname', '#zoneid').val();

        } else {
            $('#distcode').val(null).trigger('change');

        }
        $('#zoneform').validate().resetForm();

        changeButtonAction('zoneform', 'action', 'buttonaction', 'reset_button', 'display_error',
