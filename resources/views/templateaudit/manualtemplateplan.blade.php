                ...
                <form id="manualtemplateform" name="manualtemplateform">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <div class="input-group" onclick="$('#audit_date').datepicker('show');">
                                <input type="text" class="form-control datepicker" id="audit_date" name="audit_date"
                        ...
                        <div class="col-md-3 mb-3">
                            <label class="form-label">Auditor Institution Count <span id="auditor_institution_limit_label" class="text-muted"></span></label>
                            <div class="form-control d-flex align-items-center" id="auditor_institution_count_badge">
                                0
                            </div>
                        </div>
...
<script>
    ...
    let currentQuarterToDate = '';
    let currentCategoryInstitutionLimit = 0;
    ...
    function shouldShowSubcategoryInFinalizePopup() {
        const selectedCategory = $('#category').find('option:selected');
        const hasSubcategoryFlag = (selectedCategory.attr('if_subcat') || '').trim() === 'Y';
        const subcategoryValue = ($('#subcategory').val() || '').toString().trim();

        return hasSubcategoryFlag && subcategoryValue !== '';
    }
    ...
    function getSelectedInstitutionCount() {
        return getSelectedInstitutionTexts().length;
    }

    function getFinalizeLimitWarningMessage() {
        const selectedCount = getSelectedInstitutionCount();
        if (currentCategoryInstitutionLimit <= 0 || selectedCount <= currentCategoryInstitutionLimit) {
            return '';
        }

        return `This category maximum is ${currentCategoryInstitutionLimit} Per day, but you selected ${selectedCount}. Are you sure you want to finalize?`;
    }
    ...
    function updateAuditorInstitutionCountBadge() {
        ...
        const assignedCount = Number.isNaN(count) ? 0 : count;
        $('#auditor_institution_count_badge').text(
            currentCategoryInstitutionLimit > 0 ? `${assignedCount} / ${currentCategoryInstitutionLimit}` : assignedCount
        );
    }

    function updateCategoryInstitutionLimitLabel() {
        $('#auditor_institution_limit_label').text(
            currentCategoryInstitutionLimit > 0 ? `(Max: ${currentCategoryInstitutionLimit})(Per day)` : ''
        );
    }

    function applyCategoryInstitutionLimit(limit) {
        const parsedLimit = parseInt(limit, 10);
        currentCategoryInstitutionLimit = Number.isNaN(parsedLimit) ? 0 : parsedLimit;
        updateCategoryInstitutionLimitLabel();
        updateAuditorInstitutionCountBadge();
    }

    function fetchCategoryInstitutionLimit(deptcode, catcode, selectedSubcategory = '') {
        deptcode = (deptcode || '').toString().trim();
        catcode = (catcode || '').toString().trim();

        if (!deptcode || !catcode) {
            applyCategoryInstitutionLimit(0);
            return;
        }

        $.ajax({
            url: "/manualtemplate/getcategorylimit",
            type: "POST",
            data: {
                deptcode: deptcode,
                catcode: catcode,
            ...
            success: function(response) {
                applyCategoryInstitutionLimit(response.noofcolumn || 0);
            },
            error: function() {
                applyCategoryInstitutionLimit(0);
            }
    ...
    function formatAuditDateForPicker(value) {
        ...
        const normalizedValue = normalizeAuditDateValue(value);
        if (!normalizedValue) {
            return '';
        }
    ...
    function parseAuditDateToObject(value) {
        const normalizedValue = normalizeAuditDateValue(value);
        if (!normalizedValue) {
            return null;
        }
    ...
    function isBlockedAuditDate(date) {
    ...
    function findNextAvailableAuditDate(startDate, maxDate) {
        if (!(startDate instanceof Date) || Number.isNaN(startDate.getTime())) {
            return null;
        }

        const candidate = new Date(startDate.getFullYear(), startDate.getMonth(), startDate.getDate(), 12, 0, 0);
        const limit = maxDate instanceof Date && !Number.isNaN(maxDate.getTime()) ?
            new Date(maxDate.getFullYear(), maxDate.getMonth(), maxDate.getDate(), 12, 0, 0) :
            null;

        while (!limit || candidate <= limit) {
            if (!isBlockedAuditDate(candidate)) {
    ...
    function formatAuditDateForRequest(value) {
        return normalizeAuditDateValue(value);
    }

    function configureAuditDatePicker(maxDate = '', preferredDate = '') {
        const auditDateInput = $('#audit_date');
        const minDate = getTodayDateObject();
        const maxDateObject = parseAuditDateToObject(maxDate);
        const preferredDateObject = parseAuditDateToObject(preferredDate);
        const minDateDisplay = formatAuditDateForPicker(minDate);
        const maxDateDisplay = maxDateObject ? formatAuditDateForPicker(maxDateObject) : null;

        currentQuarterToDate = normalizeAuditDateValue(maxDate);

        auditDateInput.datepicker('destroy');
        auditDateInput.datepicker({
            format: 'dd/mm/yyyy',
            startDate: minDateDisplay,
            endDate: maxDateDisplay,
            autoclose: true,
            clearBtn: true,
            forceParse: false,
        ...
        let selectedDate = preferredDateObject;
        const preferredIsOutOfRange = !selectedDate || selectedDate < minDate || (maxDateObject && selectedDate > maxDateObject);
        const preferredIsBlocked = selectedDate && isBlockedAuditDate(selectedDate);
        if (preferredIsOutOfRange || preferredIsBlocked) {
            selectedDate = findNextAvailableAuditDate(minDate, maxDateObject);
        }

        const currentValue = (auditDateInput.val() || '').trim();
        if (!currentValue && !preferredDateObject) {
            selectedDate = findNextAvailableAuditDate(minDate, maxDateObject);
        }

        if (selectedDate) {
            auditDateInput.datepicker('setDate', formatAuditDateForPicker(selectedDate));
        } else {
            auditDateInput.datepicker('clearDates');
            auditDateInput.val('');
        }
    }

    function enforceAuditDateRange() {
        const auditDateInput = $('#audit_date');
        const selectedDate = parseAuditDateToObject(auditDateInput.val());
        const minDate = getTodayDateObject();
        const maxDate = parseAuditDateToObject(currentQuarterToDate);

        if (!selectedDate) {
            return;
        }

        if (selectedDate < minDate || (maxDate && selectedDate > maxDate) || isBlockedAuditDate(selectedDate)) {
            auditDateInput.datepicker('clearDates');
            auditDateInput.val('');
            if (prauditValidator) {
                prauditValidator.element('#audit_date');
            }
        }
    }

    function isFinalizeTimeAvailable() {
        const auditDateText = formatAuditDateForDisplay($('#audit_date').val());
        if (!auditDateText) {
            return true;
        }

        const normalizedDate = normalizeAuditDateValue(auditDateText);
        if (!normalizedDate) {
            return true;
        }

        const auditDate = new Date(`${normalizedDate}T12:00:00`);
        const now = new Date();
    ...
    function buildFinalizePopupContent() {
        ...
        const showSubcategory = shouldShowSubcategoryInFinalizePopup();
        const subcategoryText = showSubcategory ? getSelectedOptionText('#subcategory') : '';
        ...
        const finalizeLimitWarning = getFinalizeLimitWarningMessage();
        ...
        return `
            <div class="container-fluid px-2">
                <div class="card border-0 shadow-sm mb-3">
                    <div class="card-body">
                        <div class="row g-3">
                            ...
                            ${showSubcategory ? `<div class="col-md-6"><strong>Subcategory</strong><div>${escapeHtml(subcategoryText)}</div></div>` : ''}
                ...
                ${finalizeLimitWarning ? `
                <div class="alert alert-warning border-warning mb-3">
                    <strong>Note:</strong> ${escapeHtml(finalizeLimitWarning)}
                </div>` : ''}
    ...
    function submitManualTemplateForm(isFinalizeSubmit = false) {
        ...
        formData.set('audit_date', formatAuditDateForRequest($('#audit_date').val()));
    ...
    $(document).on('change', '#subcategory', function() {
        fetchCategoryInstitutionLimit($('#deptcode').val(), $('#category').val());
        loadInstitutions({
    ...
    $(document).on('change', '#audit_date', function() {
        enforceAuditDateRange();
        const deptcode = ($('#deptcode').val() || '').toString().trim();
        const regioncode = ($('#regioncode').val() || '').toString().trim();

        if (deptcode && regioncode) {
            loadAuditors(deptcode, regioncode, $('#auditorid').val() || '');
        }
    });

    $(document).on('input blur', '#audit_date', function() {
        enforceAuditDateRange();
    });

    $(document).on('keydown paste drop', '#audit_date', function(event) {
        event.preventDefault();
    });
    ...
    function loadQuarterByDept(deptcode) {
        $.ajax({
            ...
            success: function(response) {
                ...
                configureAuditDatePicker(response.currentquartertodate || '', $('#audit_date').val());
                if (response.quarter) {
            ...
            data: {
                ...
                success: function(response) {
                    (response.data || []).forEach(auditor => {
                        ...
                        const assignedCount = parseInt(auditor.assigned_inst_count, 10) || 0;
                        const isUnavailable = assignedCount > 0 && value !== String(selectedAuditor).trim();
                        const reserveTag = String(auditor.reservelist || '').toUpperCase() === 'Y' ?
                            ' [Reserve List]' : '';
                        const unavailableTag = isUnavailable ? ' [Already Allotted]' : '';
                        ...
                        const auditorLabel =
                            `${auditorName}${districtLabel ? ` (${districtLabel})` : ''}${reserveTag}${unavailableTag}`;
                        auditorDropdown.append(
                        `<option value="${value}" data-assigned-count="${assignedCount}" ${isUnavailable ? 'disabled' : ''} ${value === String(selectedAuditor).trim() ? 'selected' : ''}>${auditorLabel}</option>`
                        );
                    });
                    updateAuditorInstitutionCountBadge();
                },
    ...
    $(document).on('select2:select', '#category', function() {
        fetchCategoryInstitutionLimit($('#deptcode').val(), $(this).val());
    ...
    function getPrauditValidationMessages(language) {
        ...
        return {
            ...
            audit_date: {
                required: getMsg('audit_date', "Select Audit Date")
            }
        };
    }
    ...
    function populateAuditorOptions(items, selectedValue) {
        ...
        (items || []).forEach(function(auditor) {
            ...
            const assignedCount = parseInt(auditor.assigned_inst_count, 10) || 0;
            const isUnavailable = assignedCount > 0 && auditorValue !== normalizedValue;
            ...
            if (isUnavailable) {
                auditorLabel = `${auditorLabel} [Already Allotted]`;
            }

            auditorDropdown.append(
                `<option value="${auditorValue}" data-assigned-count="${assignedCount}" ${isUnavailable ? 'disabled' : ''} ${auditorValue === normalizedValue ? 'selected' : ''}>${auditorLabel}</option>`
            );
        });
    ...
    function populateprtitleForm(prtitle, optionData = {}) {
        ...
        $('#quarter_display').val(prtitle.planname || prtitle.auditquarter || prtitle.auditquartercode || prtitle
            .planmappingid || '');
        configureAuditDatePicker(optionData.currentquartertodate || '', prtitle.auditdate || '');
        ...
        fetchCategoryInstitutionLimit(deptcode, catcode);
        ...
        if (deptcode) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    fetchCategoryInstitutionLimit(deptcode, selectedCatcode || catcodeDropdown.val() || '');
                    if (typeof onLoaded === 'function') onLoaded();
                },
                error: function() {
