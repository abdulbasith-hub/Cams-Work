...
<div class="my-4">
    @foreach ($templatesData as $templateData)
        @php
            ...
            // dd($structure);
            $isEditMode = isset($dcaData) && $dcaData->statusflag === 'E';
            $isFinalMode = isset($dcaData) && $dcaData->statusflag === 'F';
        ...
        @if ($structure && $structure->deptcode === '04' && $structure->formcode === 'DCA02')
            <form id="dcaForm2" action="{{ route('templateaudit.dca_insertUpdate') }}" method="POST"
                enctype="multipart/form-data">
                <div class="card" style="border-color: #7198b9">
                    <div class="card-header card_header_color lang" key="">
                        Template Audit Details - DCA
                    </div>
                    <div class="audit-form">
                        <div class="page-header">
                            @csrf
                            @if ($isEditMode || $isFinalMode)
                                <input type="hidden" name="dcaid"
                                    value="{{ Crypt::encryptString($dcaData->dca_details_id) }}">
                            @endif
                        ...
                        <div id="template-content">
                            ...
                            <div class="row g-3">
                                <div class="col-md-6 col-lg-4">
                                    <label class="form-label lang" key=""></label>
                                    <div class="auto-field" data-english="Low Risk/Low Transaction Societies"
                                        data-tamil="குறைவிடர் - குறைந்த நடவடிக்கை சங்கங்கள்">
                                        Low Risk/Low Transaction Societies<br>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6 col-lg-4">
                                    ...
                                    <div class="auto-field"
                                        data-english="Audit Certificate for Non-credt Societies(Model)"
                                        data-tamil="கடன் கூட்டுறவு சங்கங்களுக்கான தணிக்கை சான்றிதழ்">
                                        Audit Certificate for Non-credt Societies(Model)<br>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6 col-lg-4">
                                    <label class="form-label lang" key=""></label>
                                    <div class="auto-field"
                                        data-english="Progressive Profile of the Institutions(Non-credt)"
                                        data-tamil="சங்கங்களின் சுய விவரம்(கடனற்றவை)">
                                        Progressive Profile of the Institutions(Non-credt)<br>
                                    </div>
                                </div>
                                    ...
                                    <tbody>
                                        @php
                                            ...
                                            foreach ($tableData as $pairData) {
                                                $maxlRows = max($maxlRows, count($pairData));
                                            }

                                            $amountValues = [];
                                            if (($isEditMode || $isFinalMode) && isset($dcaData->amounts)) {
                                                foreach ($dcaData->amounts as $amount) {
                                                    $value = '';
                                                    ...
                                                    $key = "header_{$amount->tmpauditheaderid}_maintype_{$amount->maintypeid}_subtype_{$amount->subtypeid}";
                                                    $amountValues[$key] = $value;
                                                }
                                            }

                                            // \Log::info('Amount values loaded:', $amountValues);
                                            ...
                                            <tr>
                                                @foreach ($pairs as $pairIndex => $pair)
                                                    @php
                                                        ...
                                                        $isMaintypeHeader =
                                                            $currentRowData &&
                                                            $currentRowData['type'] === 'maintype_header';
                                                        $isSubtype =
                                                            $currentRowData && $currentRowData['type'] === 'subtype';
                                                        $isTotal = $currentRowData['isTotal'] ?? false;

                                                        if ($isSubtype) {
                                                            $isRequired =
                                                                (!empty($currentRowData['maintypetotalflag']) &&
                                                                    $currentRowData['maintypetotalflag'] ===
                                                                        'O') ||
                                                                (empty($currentRowData['maintypetotalflag']) &&
                                                                    !$isTotal);
                                                        }
                                                    ...
                                                    <td
                                                        class="@if ($isMaintypeHeader) bg-light fw-bold @elseif ($isTotal) table-secondary fw-bold @endif">
                                                        @if ($currentRowData)
                                                            @if ($isMaintypeHeader)
                                                                <span class="lang fw-bolder"
                                                                    data-english="{{ $currentRowData['maintypeename'] ?? '' }}"
                                                                    data-tamil="{{ $currentRowData['maintypetname'] ?? '' }}">
                                                                    {{ $currentRowData['maintypeename'] ?? '' }}
                                                                </span>
                                                            @elseif ($isSubtype)
                                                                <span class="lang"
                                                                    @if ($isRequired && !$isFinalMode) aria-required="true" @endif
                                                                    data-english="{{ $currentRowData['stypeename'] ?? '' }}"
                                                                    data-tamil="{{ $currentRowData['stypetname'] ?? '' }}">
                                                    ...
                                                    @foreach ($pair['amount_headers'] as $amountHeaderIndex => $amountHeader)
                                                        @php
                                                            $headerId =
                                                            ...
                                                            if ($currentRowData) {
                                                                if ($isSubtype) {
                                                                    $key = "header_{$headerId}_maintype_{$currentRowData['maintypeid']}_subtype_{$currentRowData['subtypeid']}";
                                                                    $value = $amountValues[$key] ?? '';
                                                                    $inputName = "amount[header_{$headerId}][maintype_{$currentRowData['maintypeid']}][subtype_{$currentRowData['subtypeid']}]";
                                                                } elseif ($isMaintypeHeader) {
                                                                    $key = "header_{$headerId}_maintype_{$currentRowData['maintypeid']}";
                                                                    $value = $amountValues[$key] ?? '';
                                                                    $inputName = "amount[header_{$headerId}][maintype_{$currentRowData['maintypeid']}]";
                                                                }

                                                                if (
                                                                    ($currentRowData['temptypeid'] ?? null) == 1 &&
                                                                    $value !== ''
                                                                ) {
                                                        ...
                                                        <td
                                                            class="@if ($isMaintypeHeader) bg-light @elseif ($isTotal) table-secondary fw-bold @endif">
                                                            @if ($isMaintypeHeader && $currentRowData['temptypeid'])
                                                                <span class="text-muted"></span>
                                                            @elseif ($isSubtype)
                                                                @php
                                                                    $subTypeField =
                                                                        $currentRowData['subtemptypeid'] ?? null;
                                                                    $inputType = '';
                                                                    $step = '';
                                                                    $maxlength = '';
                                                                    $pattern = '';
                                                                        ...
                                                                        @foreach ($radioOptions as $option)
                                                                            @php
                                                                                ...
                                                                                $isChecked =
                                                                                    $value === $optionValue ||
                                                                                    $value === $option;
                                                                                $radioId = "R{$pairIndex}_{$row}_{$optionValue}";
                                                                            @endphp
                                                                            <div class="form-check">
                                                                                <input type="radio"
                                                                                    ...
                                                                                    id="{{ $radioId }}"
                                                                                    name="{{ $inputName }}"
                                                                                    data-subtemptypeid="{{ $subTypeField }}"
                                                                                    value="{{ $optionValue }}"
                                                                                    {{ $isChecked ? 'checked' : '' }}
                                                                ...
                                                                @elseif (!$isTotal && $subTypeField)
                                                                    <input type="{{ $inputType }}"
                                                                        class="form-control text-end {{ $className }}"
                                                                        name="{{ $inputName }}"
                                                                        value="{{ $value }}"
                                                                        ...
                                                                        data-temptypeid="{{ $currentRowData['temptypeid'] ?? '' }}"
                                                                        data-subtemptypeid="{{ $subTypeField }}"
                                                                        data-maintypeid="{{ $currentRowData['maintypeid'] }}"
                                                                        data-subtypeid="{{ $currentRowData['subtypeid'] }}"
                            ...
                            <div class="notes-section">
                                ...
                                <div class="col-md-4 mb-1 mt-2">
                                    ...
                                    @if (!$isFinalMode)
                                        ...
                                        <div id="view_file-list-container" class="mt-2">
                                            @if ($isEditMode && $dcaData->file_id && isset($dcaData->fileDetails))
                                                <div class="card overflow-hidden mb-3 bg-light card-fixed-width file-clickable"
                                                    id="viewfile-card-{{ $dcaData->file_id }}"
                                                    onclick="openFileInNewTab('{{ addslashes($dcaData->fileDetails->filepath) }}', '{{ addslashes($dcaData->fileDetails->filename) }}')"
                                                            ...
                                                            <div class="d-flex align-items-center">
                                                                ...
                                                                <div>
                                                                    <p class="text-dark mb-1 fw-bold">
                                                                        {{ $dcaData->fileDetails->filename }}</p>
                                                                    <small class="text-muted">
                                                                        ({{ number_format($dcaData->fileDetails->filesize / 1024, 2) }}
                                                                        KB)
                                                                    </small>
                                                                </div>
                                                            </div>
                                    ...
                                    @else
                                        @if ($dcaData->file_id && isset($dcaData->fileDetails))
                                            <div class="card overflow-hidden mb-3 bg-light card-fixed-width file-clickable"
                                                id="viewfile-card-{{ $dcaData->file_id }}"
                                                onclick="openFileInNewTab('{{ addslashes($dcaData->fileDetails->filepath) }}', '{{ addslashes($dcaData->fileDetails->filename) }}')"
                                                style="cursor: pointer; transition: all 0.3s ease;">
                                    ...
                                    <input type="hidden" id="uploadid" name="uploadid"
                                        value="{{ $isEditMode && $dcaData->file_id ? $dcaData->file_id : '' }}">
                                ...
                                <div id="auditcertificateDiv" class="hideotherdiv">
                                    ...
                                    <div class="row mb-3 align-items-start">
                                        ...
                                        @php
                                            $selectedType = $dcaData->cer_type_code ?? '01';
                                        @endphp
...
<script>
    ...
    function initializeExistingFile() {
        ...
        if (uploadId &&
            {{ $isEditMode && $dcaData->file_id && isset($dcaData->fileDetails) && $dcaData->fileDetails ? 'true' : 'false' }}
        ) {
            $('#file').hide();
    ...
    function restrictToDigits(el, minLen = 1, maxLen = 10) {
        ...
        el.addEventListener('blur', function() {
            const effectiveLength = this.value === '0' ? 1 : this.value.length;

            if (effectiveLength < minLen) {
    ...
    function formatRemarksBeforeSubmit() {
        ...
        remarksData.forEach((remark, index) => {
            ...
            document.getElementById('dcaForm2').appendChild(typeInput);

            const textInput = document.createElement('input');
            textInput.type = 'hidden';
            textInput.name = `remarks[${index}][text]`;
            textInput.value = remark.text;
            document.getElementById('dcaForm2').appendChild(textInput);
        });
    }
    ...
    function formatDecimalValue(field) {
        ...
        if (temptypeid === 1 && subtemptypeid === 1 && fieldType === 'text') {
    ...
    function calculate() {
        ...
        const maintypeTotals = new Map();

        const userEnteredValues = new Map();

        const previousYearIIIAFields = document.querySelectorAll(
            '.previous-year-input[data-calculated="false"]'
        );

        previousYearIIIAFields.forEach(field => {
            if (field && field.value) {
                const key = `${field.dataset.maintypeid}-${field.dataset.subtypeid}`;
                userEnteredValues.set(key, field.value);
            }
        });

        inputs.forEach(input => {
            const maintypeId = input.dataset.maintypeid;
            const isTotalField = input.dataset.isTotal === 'true';
            const subtemptypeid = parseInt(input.dataset.subtemptypeid);
            const temptypeid = parseInt(input.dataset.temptypeid);
            const isNumericField = subtemptypeid === 1 && temptypeid === 1;
            const isStatementIIIA = isStatementIIIAField(input);

            if (!isNumericField || isTotalField || isStatementIIIA) return;

            const value = parseFloat(input.value) || 0;

            if (!maintypeTotals.has(maintypeId)) {
                maintypeTotals.set(maintypeId, {
                    currentYear: 0,
                    previousYear: 0
                });
            }

            const totals = maintypeTotals.get(maintypeId);

            if (input.classList.contains('current-year-input')) {
                totals.currentYear += value;
            } else if (input.classList.contains('previous-year-input')) {
                totals.previousYear += value;
            }
        });
        ...
        inputs.forEach(input => {
            ...
            if (isTotalField && maintypeTotals.has(maintypeId) && !isStatementIIIA) {
                const totals = maintypeTotals.get(maintypeId);

                if (input.classList.contains('current-year-input')) {
                    input.value = totals.currentYear.toFixed(2);
                } else if (input.classList.contains('previous-year-input')) {
                    input.value = totals.previousYear.toFixed(2);
                }
            }
        });
        ...
        previousYearIIIAFields.forEach(field => {
            const key = `${field.dataset.maintypeid}-${field.dataset.subtypeid}`;
            if (field && userEnteredValues.has(key)) {
                field.value = userEnteredValues.get(key);
            }
        });

        calculatePreviousYearStatementIIIA();
    }

    function isStatementIIIAField(input) {
        const maintypeId = input.dataset.maintypeid;
        return maintypeId === "109";
    }

    function getBalanceSheetValues() {
        const currentYearNetProfit = parseFloat(
            document.querySelector('input[data-maintypeid="107"][data-subtypeid="86"].current-year-input')
            ?.value || 0
        );
        const previousYearNetProfit = parseFloat(
            document.querySelector('input[data-maintypeid="107"][data-subtypeid="86"].previous-year-input')
            ?.value || 0
        );
        const currentYearNetLoss = parseFloat(
            document.querySelector('input[data-maintypeid="133"][data-subtypeid="82"].current-year-input')
            ?.value || 0
        );
        const previousYearNetLoss = parseFloat(
            document.querySelector('input[data-maintypeid="97"][data-subtypeid="71"].previous-year-input')
            ?.value || 0
        );
    ...
    function handleStatementIIIA() {
        const balanceSheetData = getBalanceSheetValues();

        const currentYearNetProfit = balanceSheetData.currentYearNetProfit;
        const currentYearNetLoss = balanceSheetData.currentYearNetLoss;
        const previousYearNetLoss = balanceSheetData.previousYearNetLoss;

        updateStatementIIIA(currentYearNetProfit, currentYearNetLoss, previousYearNetLoss);
    }

    function updateStatementIIIA(currentYearNetProfit, currentYearNetLoss, previousYearNetLoss) {
        const statementIIIAFields = {
            netLossBroughtForward: document.querySelector(
                'input[data-maintypeid="109"][data-subtypeid="24"].current-year-input'),
            netLossCurrentYear: document.querySelector(
                'input[data-maintypeid="109"][data-subtypeid="25"].current-year-input'),
            totalNetLoss: document.querySelector(
                'input[data-maintypeid="109"][data-subtypeid="26"].current-year-input'),
            netProfitBroughtForward: document.querySelector(
                'input[data-maintypeid="109"][data-subtypeid="87"].current-year-input'),
            netLossEndOfYear: document.querySelector(
                'input[data-maintypeid="109"][data-subtypeid="88"].current-year-input'),
            finalTotal: document.querySelector(
                'input[data-maintypeid="109"][data-subtypeid="29"].current-year-input')
        };


        if (statementIIIAFields.netLossBroughtForward) {
            statementIIIAFields.netLossBroughtForward.value = previousYearNetLoss.toFixed(2);
            statementIIIAFields.netLossBroughtForward.readOnly = true;
            statementIIIAFields.netLossBroughtForward.dataset.calculated = "true";
        }

        if (statementIIIAFields.netLossCurrentYear) {
            statementIIIAFields.netLossCurrentYear.value = currentYearNetLoss.toFixed(2);
            statementIIIAFields.netLossCurrentYear.readOnly = true;
            statementIIIAFields.netLossCurrentYear.dataset.calculated = "true";
        }

        if (statementIIIAFields.totalNetLoss) {
            const totalNetLoss = previousYearNetLoss + currentYearNetLoss;
            statementIIIAFields.totalNetLoss.value = totalNetLoss.toFixed(2);
            statementIIIAFields.totalNetLoss.readOnly = true;
            statementIIIAFields.totalNetLoss.dataset.calculated = "true";
        }

        if (statementIIIAFields.netProfitBroughtForward) {
            statementIIIAFields.netProfitBroughtForward.value = currentYearNetProfit.toFixed(2);
            statementIIIAFields.netProfitBroughtForward.readOnly = true;
            statementIIIAFields.netProfitBroughtForward.dataset.calculated = "true";
        }

        if (statementIIIAFields.netLossEndOfYear) {
            const totalNetLoss = previousYearNetLoss + currentYearNetLoss;
            const finalNetLoss = Math.max(0, totalNetLoss - currentYearNetProfit);
            statementIIIAFields.netLossEndOfYear.value = finalNetLoss.toFixed(2);
            statementIIIAFields.netLossEndOfYear.readOnly = true;
            statementIIIAFields.netLossEndOfYear.dataset.calculated = "true";
        }

        if (statementIIIAFields.finalTotal) {
            const netLossEndOfYearValue = parseFloat(statementIIIAFields.netLossEndOfYear?.value || 0);
            const finalTotal = currentYearNetProfit + netLossEndOfYearValue;
            statementIIIAFields.finalTotal.value = finalTotal.toFixed(2);
            statementIIIAFields.finalTotal.readOnly = true;
            statementIIIAFields.finalTotal.dataset.calculated = "true";
        }
    }

    function calculatePreviousYearStatementIIIA() {
        const previousYearFields = {
            netLossBroughtForward: document.querySelector(
                'input[data-maintypeid="109"][data-subtypeid="24"].previous-year-input'),
            netLossCurrentYear: document.querySelector(
                'input[data-maintypeid="109"][data-subtypeid="25"].previous-year-input'),
            totalNetLoss: document.querySelector(
                'input[data-maintypeid="109"][data-subtypeid="26"].previous-year-input'),
            netProfitBroughtForward: document.querySelector(
                'input[data-maintypeid="109"][data-subtypeid="87"].previous-year-input'),
            netLossEndOfYear: document.querySelector(
                'input[data-maintypeid="109"][data-subtypeid="88"].previous-year-input'),
            finalTotal: document.querySelector(
                'input[data-maintypeid="109"][data-subtypeid="29"].previous-year-input')
        };

        const previousYearNetLossBroughtForward = parseFloat(previousYearFields.netLossBroughtForward?.value || 0);
        const previousYearNetLossCurrent = parseFloat(previousYearFields.netLossCurrentYear?.value || 0);
        const previousYearNetProfit = parseFloat(previousYearFields.netProfitBroughtForward?.value || 0);
        const previousYearNetLossEnd = parseFloat(previousYearFields.netLossEndOfYear?.value || 0);

        if (previousYearFields.totalNetLoss) {
            const totalNetLoss = previousYearNetLossBroughtForward + previousYearNetLossCurrent;
            previousYearFields.totalNetLoss.value = totalNetLoss.toFixed(2);
            previousYearFields.totalNetLoss.readOnly = true;
            previousYearFields.totalNetLoss.dataset.calculated = "true";
        }

        if (previousYearFields.finalTotal) {
            const finalTotal = previousYearNetProfit + previousYearNetLossEnd;
            previousYearFields.finalTotal.value = finalTotal.toFixed(2);
            previousYearFields.finalTotal.readOnly = true;
            previousYearFields.finalTotal.dataset.calculated = "true";
        }
    }
    ...
    function setupCalculationListeners() {
        document.querySelectorAll('.amount-input').forEach(input => {
            const isTotalField = input.dataset.isTotal === "true";
            const isCalculatedField = input.dataset.calculated === "true";
            const isStatementIIIA = isStatementIIIAField(input);

            if (isTotalField || (isStatementIIIA && input.classList.contains('current-year-input') &&
                    isCalculatedField)) {
                return;
            }
        ...
        document.querySelectorAll('.previous-year-input[data-calculated="false"]').forEach(input => {
            if (isStatementIIIAField(input)) {
                input.addEventListener('input', function() {
                    calculatePreviousYearStatementIIIA();
                });
    ...
    function initializeFormValidation() {
        ...
        $.validator.addMethod("amountField", function(value, element) {
            ...
            if (temptypeid === 1 && subtemptypeid === 1) {
        ...
        $('#dcaForm2').validate({
            ...
            messages: {
                ...
                remarks: {
                    ...
                    // maxlength: "Remarks cannot exceed 1000 characters"w
                },
                file: {
                    validFileType: function() {
                        const language = getlanguagelc();
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (finalizeBtn && !isFinalMode) {
            finalizeBtn.addEventListener('click', function(e) {
                ...
                const validator = $('#dcaForm2').validate();
