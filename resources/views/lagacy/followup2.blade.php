                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-2 ">
                                                        ...
                                                        <select class="select form-control custom-select" id="severityid"
                                                            ...
                                                            {{--
                                                         <option value="" data-en="Select Severity" data-ta="?????? ????????"></option> --}}
...
<div class="col-md-6" style="margin-left: 39% !important;"
                                                             id="file_buttonset">
                                                             <div class="d-flex align-items-center gap-6 hide_this">
                                                                 <button class="btn button_update text-light"
                                                                     id="file_save" name="file_save">Update File
                                                                 </button>

                                                             </div>
                                                         </div>
                                                    </div>
<div class="col-md-12 hide_this" id="edit_set">
    <div class="card card_border">
        <div class="card-header card_header_color">Update Legacy
            Details
        </div>
        <div class="card-body ">
            <div class="row ">
                <div class="col-md-5 mb-3">
                    <label class="form-label lang required"
                        key="{{ $instData->deptcode == '01' ? 'new_fasliyr' : 'new_audityr' }}"
                        for="validationDefault01">{{ $instData->deptcode == '01' ? 'Updated' : 'Updated' }}</label>

                    <select
                        class="form-select mr-sm-2 lang-dropdown select2"
                        id="new_yearcode" name="new_yearcode">
            ...
            <div class="row mt-1 hide_this" id="editbutton_set">
                <div class="col-md-6"
                    ...
                    <div class="d-flex align-items-center gap-6"
                        ...
                        <button type="button"
                            class="btn button_update text-light"
                            id="editbuttonaction"
                            name="editbuttonaction">Edit</button>

                      
                        <button type="button"
                            class="btn button_clear text-light bg-warning"
                            id="clearbtn"
                            onclick="clearupdatedData()">Clear</button>
  ...
  restrictSpecialChars(".removesplchar_numberwithdecimal");
        ...
        function fetch_lagacydata(followupid, slipnumber, action, createnewone, filter, yearcode) {
            ...
            $.ajax({
			  ...
			  action == 'fetch' ? fetch_lagacydata(firstItem.encrypted_followupid, firstItem
                                    .paranumber,
                                    'edit', '', $('#filter').val(), firstItem.audityear) :
                                fetch_lagacydata(followupid, slipnumber, 'edit', createnewone, $('#filter')
                                    .val(), yearcode);
        ...
        function for_newslip_resetform(clickedId) {
            ...
            enablefield()
            $('#finalisebtn').show();
	    $('#editbutton_set,#edit_set,#file_buttonset').hide();
            $('#buttonaction').show();
            $('#processname').html('')
            $('#processname').html('')
            $('#mainslipnumber').html('')
            ...
            optionText = lang === 'ta' ? '??? ??????? ?????????????????' : 'Select Category Name';


            $("#category")
                .empty()
                .append(
                    `<option value='' data-name-en="Select Category Name" data-name-ta="??? ??????? ?????????????????">${optionText}</option>`
                );

            optionText = lang === 'ta' ? '???? ??????? ?????????????????' : 'Select SubCategory ';


            $("#subcategory")
                .empty()
                .append(
                    `<option value='' data-name-en="Select SubCategory" data-name-ta="???? ??????? ?????????????????">${optionText}</option>`
                );
        }
		...
		(firstItem.duplicate_file == 'Y') ? $('#file_buttonset').show(): $('#file_buttonset').hide();
		 if (firstItem.in_para == 'Y') {
                $('#editbutton_set,#edit_set').hide()

            } else {

                $('#editbutton_set,#edit_set').show()

                var yearval = <?php echo $yearofaudit; ?>;

                console.log(yearval)
                $('#new_yearcode').empty(); // optional: clear old options

                yearval.forEach(function(item) {
                    $('#new_yearcode').append(
                        '<option value="' + item.auditperiodid + '">' +
                        item.audit_period +
                        '</option>'
                    );
                });

                $('#new_yearcode option').each(function() {
                    if ($(this).val() == firstItem.audityear) {
                        $(this).remove();
                    }
                });
                ...
                $('#buttonaction').hide();
                $('.addRowBtn').hide();
 UploadedFileList_withaction = (firstItem.duplicate_file == 'Y') ? 'edit' : 'view';
...
{            editor.setData(firstItem.remarks);
      ...
      required:true,
		...
		for (let [key, value] of formData.entries()) {
                 if (key.startsWith("slipnumber_")) {
                     formData.delete(key);
                 }
             }
             for (let [key, value] of formData.entries()) {
                 if (key.startsWith("slipid")) {
                     formData.delete(key);
                 }
             }
...
function onchange_actiontaken(val, selectedmonth, selectedyear) {
        ...
        function reset_form() {
            ...
            // Set the default option based on language
            const optionText = lang === 'ta' ? '???? ???????????? ?????????????????' : 'Select sub objection';

            $("#subobjectionid")
                .empty()
                .append(
                    `<option value='' data-name-en='Select sub objection' data-name-ta='???? ???????????? ?????????????????'>${optionText}</option>`
                );
...
$(document).on('click', '#editbuttonaction', function(event) {

            event.preventDefault();

            $("#lagacy_form").find(":input").each(function() {
                $(this).rules("remove");
            });

            $("#new_yearcode").rules("add", {
                required: true,
                messages: {
                    required: "Select the Updated Legacy year",
                }
            });

            if ($('#lagacy_form').valid()) {

                passing_alert_value('Confirmation', 'Are you sure to change the finalized legacy the data?',
        ...
        $(document).on('click', '#deletebtn', function(event) {
            ...
            passing_alert_value('Confirmation', 'Are you sure to update it as duplicate?',
                'confirmation_alert', 'alert_header', 'alert_body', 'forward_alert');
        ...
        function deletelegacydata() {
            ...
            $.ajax({
                url: '/legacy/delete_lagacydata', // For creating a new user or updating an existing one
                type: 'POST',
                data: {
                    followupid: followupid,
                    instid: instid,
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        passing_alert_value('Confirmation', response.message,
                            'confirmation_alert', 'confirmation_alertmodal', 'alert_body',
                            'confirmation_alert');
                        ...
                        $('#ok_button').off('click').on('click', function(event) {
                            ...
                            fetch_lagacydata('', '', 'fetch', 'Y', $('#filter').val());
        ...
        function updatefinalizeddata() {
$('#file_save').attr('disabled', true);
	    var typeofparacode = $('#typeofparacode').val()
            var followupid = $('#followupid').val()
            var instid = $('#instid').val()
            var parano = $('#parano').val()
            var oldyearcode = $('#yearcode').val()
            var newyearcode = $('#new_yearcode').val()


            $.ajax({
                url: '/legacy/update_lagacydata', // For creating a new user or updating an existing one
                type: 'POST',
                data: {
                    followupid: followupid,
                    instid: instid,
                    oldyearcode: oldyearcode,
                    newyearcode: newyearcode,
                    parano: parano,
		    typeofparacode: typeofparacode
                },
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
...
$('#file_save').attr('disabled', false)	
        ...
        function clearupdatedData() {
            $('#new_yearcode').val(null);
            $('#new_yearcode').select2('destroy');
            $('#new_yearcode').select2(null);
            $('#new_yearcode').select2();
        }
$(document).on('click', '#file_save', function(event) {

             event.preventDefault();

             const fileInput = $('input[name="fileupload[]"]')[0];

             if (!fileInput || fileInput.files.length === 0) {
                 passing_alert_value('Confirmation', 'File cannot be empty',
             ...
             if ($('#lagacy_form').valid()) {
                 ...
                 passing_alert_value('Confirmation', 'Are you sure to update the file?',
                     'confirmation_alert', 'alert_header', 'alert_body', 'forward_alert');
        ...
        function updateleagacy_file() {
             ...
             formData.append('deactive_fileid', deactive_fileid);
             var followupid = $('#followupid').val()
             var paranumber = $('#paranumber').val()
             $.ajax({
                 url: '/legacy/updateleagacy_file', // For creating a new user or updating an existing one
                 type: 'POST',
                 ...
                 success: function(response) {
                     ...
                     if (response.success) {
                         ...
                         $('#ok_button').off('click').on('click', function(event) {
                             ...
                             event.preventDefault();
                             fetch_lagacydata(followupid, paranumber, 'fetch', 'Y', $('#filter')
                                 .val());
