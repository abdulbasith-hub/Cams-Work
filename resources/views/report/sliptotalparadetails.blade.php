...
<div class="col-12">
    <div class="card card_border">
        <div class="card-header card_header_color">Audit Para Details</div>
        <div class="card-body">
            <form id="totalparaslipform" name="totalparaslipform" method='post'>
            @csrf
                <div class="row">
...
<div class="col-12">
    <div class="card card_border">
        <div class="card-header card_header_color lang" id="audit-slip-heading" key="">Audit Para Details
        </div>
        <div class="card-body">
            <div class="datatables">
                <div class="table-responsive hide_this" id="tableshow">
                    <table id="totalparasliptable"
                    ...
                    <div id="ParaTableContainer" class="mt-4" style="display:none;">
                        <div class="card card_border">
                            <div class="card-header card_header_color position-relative text-center">
                                <span id="ParaTitle" class="fw-bold">Para Details</span>


                                <button type="button" class="btn-close position-absolute end-0 top-50 translate-middle-y me-2"
                                    aria-label="Close" id="closeParaTable"></button>
                            </div>

                            <div class="card-body">
                                <table id="ParaReportTable"
...
$(document).on('click', '#closeParaTable', function() {
    $('#ParaTableContainer').hide();
});
...
jsonLoadedPromise.then(() => {
    ...
    var validator = $("#totalparaslipform").validate({
        ignore: [],
        rules: {
...
$(document).on('click', '.show-converttopara', function () {
    const auditscheduleid = $(this).data('auditscheduleid');
    const auditplanid = $(this).data('auditplanid');
    const createdby = $(this).data('createdby');


    $.ajax({
        url: '/fetch_converttopara',
        type: 'POST',
        data: {
        ...
        success: function (response) {
            if (response.status === 'success' && response.data.length > 0) {
                showConvertoparaTable(response.data);

                $("#ParaTableContainer").show();

                // 👉 Scroll smoothly
                $('html, body').animate({
                    scrollTop: $("#ParaTableContainer").offset().top - 50
                }, 600);


            } else {
                passing_alert_value('Info', 'No Para details found', 'confirmation_alert',
                    'alert_header', 'alert_body', 'confirmation_alert');
            }
        },
        error: function (xhr) {
            console.error('Error fetching Para details:', xhr.responseText);
            passing_alert_value('Error', 'Failed to fetch Para details', 'confirmation_alert',
                'alert_header', 'alert_body', 'confirmation_alert');
        }
    });
});
...
function showConvertoparaTable(data) {

$('#ParaTableContainer').show();
$('#ParaTitle').text("Para Details");

if ($.fn.DataTable.isDataTable('#ParaReportTable')) {
    $('#ParaReportTable').DataTable().clear().destroy();
}

let table = $('#ParaReportTable').DataTable({
    data: data,
    processing: true,
    serverSide: false,
    lengthChange: false,
...
//    dom: '<"row"<"col-6"B><"col-6 text-end"f>>rtip',
...
//         let $pagination = $('#ParaReportTable').closest('.dataTables_wrapper').find('.dataTables_paginate');
//         let $pages = $pagination.find('.paginate_button:not(.next, .previous)');

//         $pages.show();
//         $pagination.find('.next, .previous').show();
//     },
    ...
    columns: [
...

        {
            ...
            render: d => d || '-',
            className: "text-start d-none d-md-table-cell extra-column wrap-50"
        },
...
const mobileColumns = ["objectionename", "subobjectionename", "slipdetails"];
...
$(document).on('click', '.show-allparadeptails', function () {
    const mainobjectionid = $(this).data('mainobjectionid');
    const subobjectionid = $(this).data('subobjectionid');
    const auditslipid = $(this).data('auditslipid');

    $.ajax({
        url: '/fetch_allparadeptails',
        type: 'POST',
        data: { mainobjectionid, mainobjectionid,subobjectionid:subobjectionid,auditslipid:auditslipid },
...
function loadckeditorauditor(auditorreply, textareaId) {
    ...
    function renderTable(dataFromServer,language) {
        ...
        if ($.fn.DataTable.isDataTable('#totalparasliptable')) {
            $('#totalparasliptable').DataTable().clear().destroy();
        }

        // Initialize DataTable
        table = $('#totalparasliptable').DataTable({
            processing: true,
            serverSide: false,
            lengthChange: false,
            autoWidth: false,
...

            {
...

                {
                    ...
                    render: function (data, type, row) {
                        ...
                        return `
                            <b>Name:</b>${row.username} (${designation})<br>
                            <b>Mobile Number:</b> ${row.mobilenumber}<br>
                            <b>IFHRMS Number:</b> ${row.ifhrmsno}<br>
                            <b>Email:</b> ${row.email}
                        `;
                    },
                    className: 'd-none d-md-table-cell lang extra-column text-wrap'
                ...
                render: function (data, type, row) {
                    ...
                    return `
                        <button type="button"
                            class="btn btn-success btn-sm show-converttopara"
        ...
        setupMobileRowToggle(mobileColumns);
        updatedatatable("en", "totalparasliptable");

        $('#totalparasliptable').off('click').on('click', function () {
        $('#ParaTableContainer').hide();
    });
    }
    ...
    let headers = [
        ...
        { header: dtText["entry"] || "Entry Meeting Date", key: "entry" },
        { header: dtText["exit"] || "Exit Meeting Date", key: "exit" },
