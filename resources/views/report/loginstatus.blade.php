...
<div class="col-12">
    <div class="card card_border">
        <div class="card-header card_header_color">Dialy Login Status</div>
        <div class="card-body">
            <form id="loginstatus_form" name="loginstatus_form" method='post'>
                <div class="row">
                 @csrf
                 <div class="col-md-4">
                        <label class="form-label required lang" for="deptcode" key="department">Department</label>
                        <select class="form-select mr-sm-2 select2 lang-dropdown" onchange="onchange_region('region','regioncode')"  multiple="multiple" <?php echo $make_dept_disable; ?> id="deptcode"
                            name="deptcode[]">
                            ...
                            @if (!empty($dept) && count($dept) > 0)
                                @foreach ($dept as $department)
                                    ...
                                    || (!old('dept') && empty($deptcode) && $department->deptcode == '01')
                    ...
                    <table id="dailylogintable"
                        ...
                        <thead>
                            <tr>
                                ...
                                <th class="lang" key="">Username</th>
                                <th class="lang" key="">Designation</th>
                                <th class="lang" key="">Login Detals</th>
...
$(document).ready(function () {
    ...
    function handleAllOption($select) {
        ...
        if (!$select.val() || $select.val().length === 0) {
            $select.val(['A']).select2();
        }
        updateSelect($select);
    }, 0); 
    }

    ['#regioncode', '#distcode'].forEach(function (selector) {
    ...
    $('#deptcode').on('change', function () {
        ...
        if (selectedValues.includes('A')) {
            ...
            $('#deptcode option').not('[value="A"]').prop('disabled', true);
        } else {
...
jsonLoadedPromise.then(() => {
        ...
        var validator = $("#loginstatus_form").validate({
            ...
            errorPlacement: function(error, element) {
                if (element.hasClass('select2')) {
                    error.insertAfter(element.next('.select2-container'));
                        ...
                        // ✅ place error after the input-group for datepicker
    ...
    $('#buttonaction').on('click', function (event) {
        ...
        var form = $('#loginstatus_form');

        if ($("#loginstatus_form").valid()) {
    ...
    function updateTableLanguage(language) {
        if ($.fn.DataTable.isDataTable('#dailylogintable')) {
            $('#dailylogintable').DataTable().clear().destroy();
        }
    ...
    function renderTable(language) {
       ...
       const districtColumn = language === 'ta' ? 'distename' : 'distename';
        ...
        table = $('#dailylogintable').DataTable({
            processing: true,
            serverSide: false,
            lengthChange: false,
            autoWidth: false,
            ...
            columns: [{
                ...
                {
                    data: null,
                    title: columnLabels?.[usernameColumn]?.[language] || "Username",
                    render: function(data, type, row) {
                ...
                {
                    data: null,
                    title: columnLabels?.[desigColumn]?.[language] || "Designation",
                    render: function(data, type, row) {
                ...
                {
                    data: 'logintime',
                    title: columnLabels?.["logintime"]?.[language] || "Login Details",
                    render: function(data, type, row) {
                        if (!row.logintime) return '-';

                        // split datetime (assumes "YYYY-MM-DD HH:MM:SS")
                        let parts = row.logintime.split(' ');
                        let date = parts[0] || '';
                        let time = parts[1] || '';

                        if (date) {
                            let dParts = date.split('-'); // [YYYY, MM, DD]
                            date = `${dParts[2]}-${dParts[1]}-${dParts[0]}`; // DD-MM-YYYY
                        }

                        return `<b>Date :</b> ${date}<br><b>Time :</b> ${time}`;
                    },
                    className: 'text-wrap text-start'
                },
                ...
                {
                    data: 'login_count',
                    title: columnLabels?.["login_count"]?.[language] || "Login Count",
                    render: function(data, type, row) {
        ...
        updatedatatable(language, "dailylogintable");
