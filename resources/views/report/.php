 ...
 .disabled-date {
   ...
   <div class="row">
    <div class="col-12">
        <div class="card card_border">
            <div class="card-header card_header_color lang" key="checklist_head">Daily Login List</div>
            <div class="card-body">
  ...
  function initializeDataTable(language, deptcode, region, district) {
    ...
    $.ajax({
        url: "{{ route('excludeholiday.fetch') }}",
        type: "POST",
 ...
 function onchangedob() {
    ...
    const headers = [
        ...
        { header: dtText["next_schedule"] || "Next Schedule", key: "next_schedule" },
        { header: dtText["idle_days"] || "Idle Days", key: "idle_days" }
    ];
    ...
    let excelData = rawData.map(row => {
        ...
        return {
            ...
            designation: "-", // If available, adjust the selector here
            ...
            next_schedule: $row.find('td').eq(6).text().trim() || "-", // adjust index
            idle_days: $row.find('td').eq(7).text().trim() || "-" // adjust index
        };
    });
                ...
                success: function(response) {
                    ...
                    if (response.success && response.data.length > 0) {
                        ...
                        if (
                            ...
                            $('#regioncode').val(['A']).trigger('change');
                            $('#distcode').val(['A']).trigger('change');
    ...
    // Re-initialize all Select2 dropdowns
    $('#deptcode, #regioncode, #distcode').val(null).trigger('change');

    // If you want Region and District default to "All"
    $('#regioncode').val('A').trigger('change');
    $('#distcode').val('A').trigger('change');
   }
